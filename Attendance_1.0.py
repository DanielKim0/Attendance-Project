...
# TODO: email section in separate csv
# TODO: emails edit with every button
# TODO: clean up code after this is all done
...
class Attendance:
    def __init__(self):
        ...
        ttk.Radiobutton(self.root, text="Chemistry", variable=self.meeting_type, value="Chemistry").grid(row=9, column=7)
        ttk.Radiobutton(self.root, text="Physics", variable=self.meeting_type, value="Physics").grid(row=10, column=7)
        ttk.Radiobutton(self.root, text="Biology", variable=self.meeting_type, value="Biology").grid(row=11, column=7)
        ttk.Radiobutton(self.root, text="General", variable=self.meeting_type, value="General").grid(row=12, column=7)
    ...
    def save_file(self):
        ...
        if self.meeting_type == "Chemistry":
            files = ["chemistry.csv"]
        elif self.meeting_type == "Biology":
            files = ["biology.csv"]
        elif self.meeting_type == "Physics":
            files = ["physics.csv"]
        else:
            files = ["chemistry.csv", "biology.csv", "physics.csv"]
    ...
    def save_email(self):
        try:
            data = pd.read_csv("emails.csv", index_col=0)
            new_names = []
            for x in range(len(self.names_list)):
                if self.names_list[x] not in set(list(data["names"])):
                    new_names.append(x)
            new = pd.DataFrame(columns=["names", "emails"])
            for x in range(len(new_names)):
                new.loc[x] = [self.names_list[new_names[x]], self.email_list[new_names[x]]]
            print(new)
            data = pd.concat([data, new])
        except pd.errors.EmptyDataError:
            data = {"names": list(self.names_list), "emails": self.email_list}
